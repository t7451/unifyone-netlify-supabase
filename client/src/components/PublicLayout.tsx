...
export default function PublicLayout({ children }: PublicLayoutProps) {
  ...
  return (
    <div style={{ backgroundColor: "#020202", color: "#F0E8D0", minHeight: "100vh" }}>
          ...
          <Link href="/">
            <div className="flex items-center gap-3 cursor-pointer group">
              <div className="relative w-7 h-7 flex items-center justify-center shrink-0">
                <div className="absolute inset-0 transition-colors duration-300" style={{ border: "1px solid rgba(212,168,67,0.4)" }} />
              ...
              <div>
                <span className="font-cinzel text-sm font-700 tracking-widest group-hover:text-amber-400 transition-colors" style={{ color: "#D4A843", letterSpacing: "0.2em" }}>
                  UNIFYONE
                </span>
              ...
              <Link key={link.href} href={link.href}>
                <span
                  ...
                  style={{
                    ...
                    borderBottom: isActive(link.href) ? "1px solid rgba(212,168,67,0.5)" : "1px solid transparent",
                    paddingBottom: "2px",
                  }}
                  onMouseEnter={e => { if (!isActive(link.href)) (e.currentTarget as HTMLElement).style.color = "#D4A843"; }}
                  onMouseLeave={e => { if (!isActive(link.href)) (e.currentTarget as HTMLElement).style.color = "#5A5A5A"; }}
                >
                  {link.label}
                </span>
              </Link>
      ...
      {/* ── FOOTER ─────────────────────────────────────────────────────── */}
      <footer style={{ borderTop: "1px solid rgba(212,168,67,0.08)", backgroundColor: "#020202" }}>
      ...
      {/* Mobile sticky CTA */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 p-4" style={{ backgroundColor: "rgba(2,2,2,0.97)", borderTop: "1px solid rgba(212,168,67,0.1)" }}>
        <a href={getLoginUrl()} className="btn-illuminate block text-center w-full" style={{ padding: "0.875rem", fontSize: "0.7rem" }}>
