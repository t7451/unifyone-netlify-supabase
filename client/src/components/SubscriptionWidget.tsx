...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; className: string }> = {
    ...
    trialing: { label: "Trial", className: "bg-cyan-500/20 text-cyan-400 border-cyan-500/30" },
    past_due: { label: "Past Due", className: "bg-amber-500/20 text-amber-400 border-amber-500/30" },
    cancelled: { label: "Cancelled", className: "bg-red-500/20 text-red-400 border-red-500/30" },
    none: { label: "Free", className: "bg-slate-500/20 text-slate-400 border-slate-500/30" },
  };
...
export function SubscriptionWidget() {
  ...
  const { status, plan, trialDaysLeft, subscriptionCurrentPeriodEnd, usage } = data;
  ...
  return (
    <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 overflow-hidden">
      ...
      {data.tenantStatus === "trial" && trialDaysLeft !== null && trialDaysLeft <= 7 && (
        <div
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              {periodEndStr && (
                <p className="text-[11px] text-slate-500 mt-0.5">Renews {periodEndStr}</p>
              )}
            </div>
          </div>
        ...
        {(status === "none" || data.tenantStatus === "trial") && trialDaysLeft !== null && trialDaysLeft > 7 && (
        ...
        {usage && (
          <div className="space-y-2.5 pt-1">
            <p className="text-[11px] font-medium text-slate-500 uppercase tracking-wider">Usage</p>
        ...
        {/* CTA */}
        {(status === "none" || status === "cancelled" || data.tenantStatus === "trial") && (
          <Button
            size="sm"
