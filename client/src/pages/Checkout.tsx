...
export default function Checkout() {
  ...
  // PATCHED: auth state no longer needed in this file — tenant/user come
  // from the JWT cookie inside the /api/stripe/create-checkout handler.
  ...
  const handleStripeCheckout = async () => {
    // PATCHED:USE_TRPC_FOR_STRIPE
    // Use the authenticated tRPC mutation so tenant/user are taken from the
    // JWT, not the request body, and so a real DB-backed plan price is used.
    setIsProcessing(true);
    try {
      ...
      toast.error(`Stripe error: ${err.message ?? "unknown"}`);
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleShopifyCheckout = () => {
    if (!shopifyCheckoutUrl) {
      toast.error(
        "Shopify checkout URL not configured. Go to Integrations → Shopify to set it."
      );
      return;
    }
          ...
          <button
            onClick={() =>
              navigate(
                planSlug
                  ? "/"
                  : landingPathForProduct(meQuery.data?.primaryProduct)
              )
            }
        ...
        {planSlug && (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-center gap-4 mt-2">
              {["SSL Encrypted", "PCI Compliant", "Cancel Anytime"].map(
                item => (
                  <div
                    key={item}
        ...
        {!planSlug && (
          <>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            ...
                            style={{
                              backgroundColor: rail.available
                                ? rail.color + "20"
                  ...
                  {paypalConfigured ? (
                    <>
                      ...
                      <p className="text-gray-600 text-xs text-center mt-2">
                        By clicking PayPal, you authorize this purchase. PayPal
                    ...
                    <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                      ...
                      <div>
                        ...
                        <p className="text-amber-400/70 text-xs mt-0.5">
                          Add PAYPAL_CLIENT_ID and PAYPAL_CLIENT_SECRET in
                          Settings → Secrets to enable PayPal payments.
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <div className="space-y-4">
                  ...
                  {shopifyCheckoutUrl ? (
                    <>
                      ...
                      <p className="text-gray-600 text-xs text-center">
                        Opens your Shopify store in a new tab. Payment is
                    ...
                    <div className="space-y-3">
                      <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                        ...
                        <div>
                          <p className="text-amber-300 text-sm font-medium">
                            Shopify checkout URL not set
                          </p>
                          <p className="text-amber-400/70 text-xs mt-0.5">
                            Go to Integrations → Shopify and paste your store's
                            checkout or product URL.
                          </p>
                        </div>
                      </div>
                      <Button
              ...
              {selectedRail === "square" && (
                <div className="space-y-4">
                  <div className="flex items-center gap-2 text-gray-400 text-xs mb-3">
                    ...
                    <span>
                      Secured by Square — PCI DSS compliant payment processing
                    </span>
                  </div>
                  <Button
                  ...
                  <p className="text-gray-600 text-xs text-center">
                    You'll be redirected to Square's secure hosted checkout
                    page.
                  </p>
                </div>
              )}
            ...
            <div className="flex items-center justify-center gap-4 mt-4">
              {["SSL Encrypted", "PCI Compliant", "Fraud Protected"].map(
                item => (
                  <div
                    key={item}
                    className="flex items-center gap-1 text-gray-600 text-xs"
