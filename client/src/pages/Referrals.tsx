...
const CREDIT_RATES = [
  ...
  { action: "Someone signs up via your link", credits: 500, note: "Per signup" },
  { action: "Referred user converts to paid", credits: 2000, note: "Per conversion" },
...
export default function Referrals() {
  ...
  const { data: transactions } = trpc.referral.getTransactions.useQuery({ limit: 20 });

  const redeemCredits = trpc.referral.redeemCredits.useMutation({
    onSuccess: (data) => {
      refetchStats();
      setRedeemDialogOpen(false);
      toast.success(`Redeemed ${data.creditsRedeemed} credits — $${data.dollarValue} will be applied to your next invoice.`);
    },
        ...
        <div className="relative grid md:grid-cols-3 gap-6 items-center">
          <div className="md:col-span-2 space-y-3">
            <div className="flex items-baseline gap-2">
              <span className="text-5xl font-bold text-white">{creditBalance.toLocaleString()}</span>
              <span className="text-violet-300 text-lg">credits</span>
            </div>
            <p className="text-violet-200/70 text-sm">
              ≈ <span className="text-white font-semibold">${dollarValue}</span> off your next invoice
              <span className="text-violet-300/50 ml-1">(100 credits = $1)</span>
            </p>
            <div className="flex flex-wrap gap-3 pt-1">
              ...
              <Button size="sm" variant="outline" className="border-violet-500/30 text-violet-300 hover:bg-violet-500/10" onClick={copyLink}>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: "Link Clicks", value: stats?.totalClicks ?? 0, icon: TrendingUp },
              { label: "Signups", value: stats?.signups ?? 0, icon: Users },
              { label: "Conversions", value: stats?.conversions ?? 0, icon: CheckCircle2 },
              { label: "Total Earned", value: `${(stats?.totalCreditsEarned ?? 0).toLocaleString()} cr`, icon: Star },
            ].map(s => (
              <div key={s.label} className="rounded-lg bg-white/5 border border-white/10 p-2.5 text-center">
      ...
      {referralUrl && (
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>Share this link — earn 500 credits per signup, 2,000 per paid conversion</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex gap-2">
              <Input value={referralUrl} readOnly className="text-sm font-mono bg-muted/50" />
            ...
            <p className="text-xs text-muted-foreground mt-2">
              FTC Disclosure: Recipients see "I earn credits when you sign up" on all templates below.
            </p>
          </CardContent>
        </Card>
      )}
        ...
        <div className="lg:col-span-3 space-y-4">
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>One-click share with pre-written, FTC-compliant posts</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {SHARE_TEMPLATES.map(tmpl => {
                const storeName = (user as any)?.name ?? "my store";
                const text = tmpl.template(referralUrl || "https://unifyone.com", storeName);
                return (
                  <div key={tmpl.platform} className={`rounded-lg border p-4 space-y-3 ${tmpl.bg}`}>
                    <div className={`flex items-center gap-2 font-medium text-sm ${tmpl.color}`}>
                      <tmpl.icon className="h-4 w-4" />
                      {tmpl.platform.charAt(0).toUpperCase() + tmpl.platform.slice(1)}
                      {tmpl.platform === "linkedin" && (
                        <Badge variant="outline" className="text-[10px] border-blue-500/30 text-blue-400 ml-auto">+75 cr</Badge>
                      )}
                      {tmpl.platform === "instagram" && (
                        <Badge variant="outline" className="text-[10px] border-pink-400/30 text-pink-400 ml-auto">+60 cr</Badge>
                      )}
                      {(tmpl.platform === "twitter" || tmpl.platform === "facebook") && (
                        <Badge variant="outline" className="text-[10px] border-sky-400/30 text-sky-400 ml-auto">+50 cr</Badge>
                      )}
                    </div>
                    <p className="text-xs text-foreground/80 line-clamp-4 whitespace-pre-line">{text}</p>
                    <div className="flex gap-2">
                      <Button
                        size="sm"
                        variant="outline"
                      ...
                      {tmpl.platform === "instagram" && (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            toast.success("Caption copied! Paste it in your Instagram app. +60 credits earned.");
                          }}
                        >
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {CREDIT_RATES.map((rate, i) => (
                <div key={i} className="flex items-center justify-between py-1.5 border-b border-border/50 last:border-0">
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground">{rate.note}</p>
                  </div>
                  {rate.credits > 0 ? (
              ...
              <p className="text-[10px] text-muted-foreground pt-1">
                100 credits = $1 off your next subscription invoice. Min redemption: 100 credits.
              </p>
            </CardContent>
          </Card>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            <div className="rounded-lg bg-muted/50 p-4 space-y-1">
              ...
              <p className="text-3xl font-bold">{creditBalance.toLocaleString()} <span className="text-lg text-muted-foreground">credits</span></p>
              <p className="text-sm text-muted-foreground">≈ ${dollarValue} value</p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Credits to redeem</label>
              <div className="flex gap-2">
                {[100, 500, 1000, creditBalance].filter((v, i, a) => a.indexOf(v) === i && v <= creditBalance && v > 0).map(v => (
                  <Button
                    key={v}
                    size="sm"
            ...
            <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-3 text-sm">
              Redeeming <strong>{redeemAmount} credits</strong> = <strong>${(redeemAmount / 100).toFixed(2)}</strong> off your next invoice
            </div>
            <Button
              className="w-full"
              disabled={redeemAmount < 100 || redeemAmount > creditBalance || redeemCredits.isPending}
              onClick={() => redeemCredits.mutate({ amount: redeemAmount })}
            >
              {redeemCredits.isPending ? "Processing..." : `Redeem ${redeemAmount} Credits`}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
