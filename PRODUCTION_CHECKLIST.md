...
| `SUPABASE_SECRET_KEY`          | Supabase secret key (`sb_secret_…`) for the server-side credit-metering/billing layer (legacy `SUPABASE_SERVICE_ROLE_KEY` also accepted) | Supabase Dashboard → Settings → API keys                                              |
