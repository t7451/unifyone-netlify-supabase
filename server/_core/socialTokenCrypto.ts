/**
 * AES-256-GCM at-rest encryption for social platform tokens
 * (access tokens, refresh tokens, app passwords).
 *
 * Wire format (base64): iv (12B) || authTag (16B) || ciphertext
 * Rotation: bump CIPHER_VERSION + add SOCIAL_TOKEN_ENC_KEY_V<n>
 *
 * Mirrors server/_core/shopifyTokenCrypto.ts. Keyed independently via
 * SOCIAL_TOKEN_ENC_KEY so social-token compromise is isolated from Shopify.
 */
import {
  createCipheriv,
  createDecipheriv,
...
function deriveKey(material: string): Buffer {
  if (!material || material.length < 32) {
    throw new Error(
      "SOCIAL_TOKEN_ENC_KEY must be set to ≥32 chars. Generate with: openssl rand -hex 32"
    );
  }
...
function getKey(version: number = CIPHER_VERSION): Buffer {
  if (version === 1) return deriveKey(process.env.SOCIAL_TOKEN_ENC_KEY ?? "");
  return deriveKey(process.env[`SOCIAL_TOKEN_ENC_KEY_V${version}`] ?? "");
}
