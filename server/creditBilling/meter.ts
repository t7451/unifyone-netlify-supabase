/**
 * server/creditBilling/meter.ts — Unified credit metering use-cases.
 *
...
import {
  getSupabase,
  getStripe,
  consumeCreditsWithMeter,
  fetchPendingOverages,
  fetchActiveSubscriptionCustomer,
  fetchPendingOverageUserIds,
  markOverageReported,
  stampUsageEventInvoiceItem,
  markOverageFailed,
...
export async function meterCredits(
  ...
  const { data, error } = await consumeCreditsWithMeter(supabase, {
    userId,
    amount: input.amount,
    source: input.source,
    action: input.action,
...
export async function flushUserOverages(
  ...
  const { data: pending, error } = await fetchPendingOverages(supabase, userId);
  ...
  const { data: sub } = await fetchActiveSubscriptionCustomer(supabase, userId);
  ...
  for (const row of pending) {
    try {
      const item = await createOverageInvoiceItem(stripe, {
      ...
      await markOverageReported(supabase, row.id, item.id);

      // Also stamp the usage event with the invoice item id
      if (row.usage_event_id) {
        await stampUsageEventInvoiceItem(supabase, row.usage_event_id, item.id);
      }
      ...
      await markOverageFailed(supabase, row.id, errMsg(err));
      failed++;
    }
  }
...
export async function flushAllOverages(): Promise<{
  ...
  const { data } = await fetchPendingOverageUserIds(supabase);
