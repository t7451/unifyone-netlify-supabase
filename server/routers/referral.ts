...
export const referralRouter = router({
      ...
      if (existing.length > 0) {
        await db
          ...
          .set({
            ...
            referredEmail: ctx.user.email ?? undefined,
          })
  ...
  awardSocialShare: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const db = await requireDb();
      const key =
        `social_share_${input.platform}` as keyof typeof CREDIT_AWARDS;
  ...
  getStats: protectedProcedure.query(async ({ ctx }) => {
    ...
    const conversions = myReferrals.filter(
  ...
  redeemCredits: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (balance < input.amount) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: `Insufficient credits. You have ${balance}, need ${input.amount}.`,
        });
      }
