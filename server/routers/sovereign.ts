...
export const sovereignRouter = router({
  ...
  joinWaitlist: publicProcedure
    .input(z.object({
      ...
      monthlyRevenue: z.enum(["pre_revenue", "under_5k", "5k_25k", "25k_100k", "over_100k"]).optional(),
    ...
    .mutation(async ({ input }) => {
      ...
      const existing = await db.select({ id: sovereignWaitlist.id, position: sovereignWaitlist.position })
      ...
      const [{ total }] = await db.select({ total: count() }).from(sovereignWaitlist);
  ...
  listWaitlist: adminProcedure
    .input(z.object({
      status: z.enum(["pending", "contacted", "qualified", "converted", "rejected"]).optional(),
    ...
    .query(async ({ input }) => {
      ...
      const query = db.select().from(sovereignWaitlist).orderBy(desc(sovereignWaitlist.createdAt)).limit(input.limit).offset(input.offset);
  ...
  updateStatus: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      await db.update(sovereignWaitlist)
