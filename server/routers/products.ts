...
import { z } from "zod";
import {
  createProduct, deleteProduct, getCategories, getInventory,
  getLowStockProducts, getProductById, getProducts, updateProduct, upsertInventory, createCategory
} from "../db";
...
export const productsRouter = router({
  ...
  create: protectedProcedure.input(z.object({
    ...
    if (product && input.trackInventory) {
      await upsertInventory(tenantId, product.id, input.initialStock, input.lowStockThreshold);
    }
    return product;
  }),
  ...
  update: protectedProcedure.input(z.object({
    ...
    const tenantId = requireTenant(ctx.user.tenantId);
    const { id, quantity, lowStockThreshold, price, compareAtPrice, costPrice, ...rest } = input;
