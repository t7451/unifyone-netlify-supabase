...
export default async (_req: Request) => {
  const env = {
    ...
    // The digest is an operational, dynamically-rendered HTML email, so it
    // goes through Resend (the transactional provider the rest of the app
    // already uses for auth, drip, invites, blueprint, and deploy alerts).
    // MailerLite is reserved for marketing campaigns and rejects arbitrary
    // API-submitted HTML on the current plan.
  ...
  try {
    ...
    if (env.RESEND_API_KEY && env.DIGEST_TO_EMAIL && env.DIGEST_FROM_EMAIL) {
      ...
      logger.info(
        ...
        "Digest email skipped (Resend key, recipient, or sender missing); logging content"
      );
    }
...
async function sendDigestEmail(params: {
  ...
  // Resend transactional send — same endpoint/pattern as the deploy notifier
  // and the rest of the app's operational email. Resend accepts arbitrary HTML
  // bodies, which the dynamically-rendered digest requires.
